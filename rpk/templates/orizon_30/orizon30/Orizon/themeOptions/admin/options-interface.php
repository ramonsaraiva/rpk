...
<script type="text/javascript">
		...
		// Start jQuery goodness
			jQuery('#itoggle input#<?php echo $value['id']; ?>').iToggle({
				easing: 'easeOutExpo',
				onClickOn: function(){
					jQuery('#console').show().css({opacity:0}).animate({opacity:1},400);
					statusUpdate('Console on');
				},
				onClickOff: function(){
					statusUpdate('Console off');
					jQuery('#console').animate({opacity:0},400);
				}
			});
			...
			function statusUpdate(text){
				jQuery('#console').prepend('<p>'+text+'</p>');
			}

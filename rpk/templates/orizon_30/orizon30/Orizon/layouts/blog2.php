 ...
 <article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
	<header class="entry-header">
		<?php if ( is_sticky() ) : ?>
			...
			<?php else : ?>
			<h1 class="b2-entry-title"><a href="<?php the_permalink(); ?>" title="<?php printf( esc_attr__( 'Permalink to %s', 'gp' ), the_title_attribute( 'echo=0' ) ); ?>" rel="bookmark"><?php the_title(); ?></a></h1>
			<?php endif; ?>
			
<div class="b2-entry-meta">
...
<?php comments_number( __( 'No Comments', 'gp'), __( '1 Comment', 'gp' ), __( '% Comments', 'gp' ) ); ?>
		&nbsp; &nbsp; <?php edit_post_link( __( 'Edit', 'gp' ), '<span class="b2-edit-link">', '</span>' ); ?>
</div>			
						
	</header><!-- .entry-header -->
<div class="b2-post-wrapper">	
<?php if (has_post_thumbnail()) : ?>
	<?php
    $thumb = get_post_thumbnail_id(); 
    $postimage = vt_resize($thumb, '', 160, 115, true );
?>    
<div class="b2-image-wrapper" style="background-color:<?php echo of_get_option('post_image_background'); ?>;">
	<span class="b2-overlay"></span>
			<?php if ( is_home() ) : ?>
	...
	<?php endif; ?>
</div><!-- .b2-image-wrapper -->
<?php endif; ?>


	<div class="b2-entry-summary b2">

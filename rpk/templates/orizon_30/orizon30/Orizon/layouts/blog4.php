 ...
 <article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
	<header class="entry-header">
		<?php if ( is_sticky() ) : ?>
			...
			<?php else : ?>
			<h1 class="b4-entry-title"><a href="<?php the_permalink(); ?>" title="<?php printf( esc_attr__( 'Permalink to %s', 'gp' ), the_title_attribute( 'echo=0' ) ); ?>" rel="bookmark"><?php the_title(); ?></a></h1>
			<?php endif; ?>
	</header><!-- .entry-header -->
...
?>    
	...
	<div class="b4-postdate" style=" color:<?php echo of_get_option('post_date_text'); ?>;">


		<span class="b4-monthyear"><?php echo get_the_date('M'); ?><br /><?php echo get_the_date('Y'); ?></span><span class="b4-day"><?php echo get_the_date('d'); ?></span>
...
<?php comments_number( __( 'No Comments', 'gp'), __( '1 Comment', 'gp' ), __( '% Comments', 'gp' ) ); ?><br  />
	...
	<?php the_category(', '); ?><br  />
<?php edit_post_link( __( 'Edit', 'gp' ), '<span class="b4-edit-link">', '</span>' ); ?>
</div>

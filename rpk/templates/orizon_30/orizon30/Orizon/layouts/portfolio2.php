<?php 
/**
 * Concept: Portfolio Page with 4 columns
...
<p><?php echo of_get_option('portfolio_intro'); ?></p>
...
<div id="gp-portfolio">
	...
	$postimage = vt_resize($thumb, '', 460, $thumbheight, true );
	$pfdesc = get_post_meta($post->ID,'Image Description', true); // this is a custom field for the prettyPhoto popup
	?>
       
	<li class="pf-itemwrap glow column-<?php echo $count; ?> <?php if($count  % 2 == 0) echo 'last' ?>">
	
	<div class="pf-imagewrap" style="height:<?php echo of_get_option('portfolio_thumbnail_height'); ?>px;" >
	<div class="pf-overlay"></div>
	<a rel="prettyPhoto" href="<?php $thumbnail_id=get_the_post_thumbnail($post->ID); preg_match ('/src="(.*)" class/',$thumbnail_id,$link); echo $link[1]; ?>" title="<?php echo $pfdesc; ?>">
	<img src="<?php echo $postimage[url]; ?>" alt="<?php the_title(); ?>" width="<?php echo $postimage[width]; ?>" height="<?php echo $postimage[height]; ?>" style="border-bottom:6px solid <?php echo of_get_option('portfolio_img_bottom'); ?>;" /></a>
